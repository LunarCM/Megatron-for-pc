...
def get_train_data_iterator() -> Iterator:
    ...
    tokenizer = MegatronTokenizer.from_pretrained(
        ...
        # metadata_path={
        #     "library": "megatron",
        #     "vocab_file": "/home/lxk/PycharmProjects/My_megatron/simplestories-4k-megatron/gpt2-vocab.json",
        #     "merge_file": "/home/lxk/PycharmProjects/My_megatron/simplestories-4k-megatron/gpt2-merges.txt",
        # }
        metadata_path={
            "library": "huggingface",
            "model_name": "/home/lxk/PycharmProjects/My_megatron/simplestories-4k-megatron"
        }
    )
    # tokenizer = MegatronTokenizer.from_pretrained(
    #     tokenizer_path="GPT2BPETokenizer",
    ...
    #     }
    # )
    # 或者使用 HuggingFaceTokenizer:
    # from megatron.core.tokenizers import HuggingFaceTokenizer
    # tokenizer = HuggingFaceTokenizer("/path/to/tokenizer_model", vocab_size=50257)
    ...
    config = GPTDatasetConfig(
        ...
        tokenizer=tokenizer,
        # 新增：指定数据集前缀
        data_prefix={"train": "/home/lxk/PycharmProjects/My_megatron/simplestories-4k-megatron"},
        # 可选：若数据已经预先切分，可指定 split 比例
        # split="98,2,0",   # 训练:验证:测试
    )

    datasets = BlendedMegatronDatasetBuilder(
        GPTDataset,            # 改为 GPTDataset
        [1000, None, None],    # 训练集大小（可为 None 自动读取）
        lambda: True,          # 是否允许任意序列长度（通常 True）
        config
    ).build()
